...
# output filename
if [ "$SHELL" -eq 1 ]; then
    OUTNAME=solvation_orientation
else
    OUTNAME=solvation_orientation_${SHELL}
fi
...
for ((k = k_0; k < (k_0 + N); k++)); do
    ...
    if [ "$SLURM" == "y" ]; then
        sbatch --job-name="$JOB_NAME" -t "$SLURMTIME" run_so.sh "${SIMPATH}" "${STDNAME}" "${OUTNAME}" "${BINS}" "${SHELL}" "${START}" "${END}" "${SKIP}" "${CATION[@]}" "${ANIONS[@]}"
    else
        bash run_so.sh "${SIMPATH}" "${STDNAME}" "${OUTNAME}" "${BINS}" "${SHELL}" "${START}" "${END}" "${SKIP}" "${CATION[@]}" "${ANIONS[@]}"
    fi
done
