...
for ((k = k_0; k < (k_0 + N); k++)); do
    ...
    if [ "$SLURM" == "y" ]; then
        sbatch --job-name="$JOB_NAME" -t "$SLURMTIME" run_iid.sh "${SIMPATH}" "${STDNAME}" "${OUTNAME}" "${START}" "${END}" "${SKIP}" "${CATION[@]}" "${ANIONS[@]}"
    else
        bash run_iid.sh "${SIMPATH}" "${STDNAME}" "${OUTNAME}" "${START}" "${END}" "${SKIP}" "${CATION[@]}" "${ANIONS[@]}"
    fi
done
