...
for ((k = k_0; k < (k_0 + N); k++)); do
    ...
    if [ "$SLURM" == "y" ]; then
        sbatch --job-name="$JOB_NAME" -t "$SLURMTIME" run_2d.sh "${SIMPATH}" "${STDNAME}" "${OUTNAME}" "${CATION[0]}" "${LIMIT}" "${BINS_R}" "${BINS_T}" "${START}" "${END}" "${SKIP}"
    else
        bash run_2d.sh "${SIMPATH}" "${STDNAME}" "${OUTNAME}" "${CATION[0]}" "${LIMIT}" "${BINS_R}" "${BINS_T}" "${START}" "${END}" "${SKIP}"
    fi
done
