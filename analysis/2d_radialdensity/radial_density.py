...
def radial_density(simpath, confname, trajname, output, atoms, limit, bins, window=[[0,0],[0,0],[0,0]], start=0, end=-1, skip=1):
    ...
    binw_t = 2 * np.pi / bins[1]
    edges_t = np.array([binw_t * i - np.pi for i in range(bins[1]+1)])
    centers_t = np.array([binw_t * (i + 0.5) - np.pi for i in range(bins[1])])
    cnt = np.zeros((bins[0],bins[1]))
    ...
    vol = lambda r1, r2: (2 / 3) * binw_t * (r2 ** 3 - r1 ** 3)
    ...
    for ts in system.trajectory[start:end:skip]:
        ...
        for j in range(len(atom_coord[ion_to_analyze][0])):
            ...
            for i in range(len(atom_pos[0][0])):
                ...
                if idx_r < bins[0]:
                    # idx_t = edges_t.searchsorted(math.atan2(y2-y1,x2-x1),'right')-1
                    # if dis < 2.5:
                    #     print(x1,z1)
                    #     print(math.atan2(x1-x2,z1-z2))
                    if x1-x2 > box_dims[0]/2:
                        x1 -= box_dims[0]
                    elif x1-x2 < -box_dims[0]/2:
                        x1 += box_dims[0]
                    if z1-z2 > box_dims[2]/2:
                        z1 -= box_dims[2]
                    elif z1-z2 < -box_dims[2]/2:
                        z1 += box_dims[2]
                    idx_t = edges_t.searchsorted(math.atan2(x1-x2,z1-z2), 'right')-1
                    if idx_t == bins[1]:
                        idx_t -= 1
